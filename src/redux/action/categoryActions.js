...
export const fetchCategoryStories = (categoryName,pageNumber,pagesize) => async (dispatch) => {
  try {
    dispatch({ type: TOGGLE_CATEGORY_FETCHING_STATE });

    const response = await axios(
      `https://cors-anywhere.herokuapp.com/http://newsapi.org/v2/top-headlines?country=in&category=${categoryName}${
